...
test("browser exit preserves the PTY while host crash cleanup ends its process group", async () => {
  ...
  try {
    ...
    await waitUntil(
      async () => {
        const state = await reattachedBrowser.page.evaluate(() => window.__yaadeTest?.getState());
        const sameTerminal = state?.muxTerminals?.find((item) => item.id === terminal.id);
        return (
          sameTerminal?.output.processIdentity?.pid === pid &&
          sameTerminal.output.processState === "running"
        );
      },
      30_000,
      "reattached terminal process identity",
    );
    expect(processAlive(pid)).toBe(true);
