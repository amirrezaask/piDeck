...
fn main() {
    ...
    let generated = rustfmt(
        &bindings.to_string().replace(
            "        unsafe {\n            ::std::ptr::write_bytes(s.as_mut_ptr(), 0, 1);",
            "        // SAFETY: Bindgen emits Default only for C structs whose all-zero byte pattern is valid.\n        unsafe {\n            ::std::ptr::write_bytes(s.as_mut_ptr(), 0, 1);",
        ),
    );
    if env::args().any(|arg| arg == "--check") {
        ...
        assert_eq!(
            generated, expected,
            "checked bindings are stale; regenerate them from the pinned public headers"
        );
    } else {
        std::fs::write(&out, generated).unwrap_or_else(|error| {
            panic!("failed to write bindings to {}: {error}", out.display())
        });
    }
}
...
fn rustfmt(source: &str) -> String {
    let mut child = Command::new("rustfmt")
        ...
        .stdout(Stdio::piped())
        .spawn()
        .expect("rustfmt must be installed to regenerate bindings");
    child
        .stdin
        .take()
        .expect("rustfmt stdin must be available")
        .write_all(source.as_bytes())
        .expect("failed to send generated bindings to rustfmt");
    let output = child
        .wait_with_output()
        .expect("failed to wait for rustfmt");
    assert!(
        output.status.success(),
        "rustfmt rejected generated bindings"
    );
