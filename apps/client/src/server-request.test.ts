...
describe('Electron server request broker', () => {
  ...
  it.each([
    null,
    [],
    { serverId: '', path: '/v1/health', method: 'GET' },
    { serverId: 'remote', path: '/v1/health', method: 'GET', headers: null },
    { serverId: 'remote', path: '/v1/health', method: 'GET', headers: [] },
    { serverId: 'remote', path: '/v1/health', method: 'GET', body: 42 },
  ...
  it('resolves API paths and forwards only explicitly allowed headers', () => {
    ...
    expect(prepared.target.toString()).toBe('https://agents.example.com/v1/agents?limit=25');
  ...
  it.each([
    'https://evil.example/v1/agents',
    '//evil.example/v1/agents',
    '/v1/../../admin',
    '/v1/%2e%2e/admin',
    '/v1\\health',
    '/v1/health#fragment',
  ...
  it('rejects unsupported methods and oversized bodies', () => {
    expect(() =>
      prepareServerRequest(
        { serverId: 'remote', path: '/v1/health', method: 'CONNECT' },
        'https://agents.example.com',
      ),
