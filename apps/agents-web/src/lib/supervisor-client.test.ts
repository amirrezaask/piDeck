...
describe('SupervisorClient', () => {
  ...
  it('creates an agent with validated input and service authentication', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      'http://supervisor.test/agents/v1/agents',
  ...
  it('creates and lists projects with validated paths', async () => {
    ...
    expect(fetcher.mock.calls.map(([url]) => url)).toEqual([
      '/supervisor-api/agents/v1/projects',
      '/supervisor-api/agents/v1/projects?limit=12',
    ]);
  });
  ...
  it('loads persisted run attachments', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe('/supervisor-api/agents/v1/runs/run-123/attachments');
  });
  ...
  it('loads a bounded latest or historical run-event page', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe(
      `/supervisor-api/agents/v1/runs/${runId}/events?afterSequence=0&limit=1&beforeSequence=8`,
    );
  });
  ...
  it('loads a command receipt by idempotency key', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe('/supervisor-api/agents/v1/command-receipts/retry-key');
  });
  ...
  it('updates a project by id', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      `/supervisor-api/agents/v1/projects/${project.id}`,
      expect.objectContaining({
        method: 'PATCH',
  ...
  it('lists configured models before agent creation', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe('/supervisor-api/agents/v1/models');
  ...
  it('iterates all run pages and rejects a repeated cursor', async () => {
    ...
    expect(fetcher.mock.calls.map(([url]) => url)).toEqual([
      '/agents/v1/runs?limit=1&status=running',
      '/agents/v1/runs?limit=1&status=running&cursor=page-2',
    ]);
  ...
  it('lists composer suggestions for a working directory', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe(
      '/supervisor-api/agents/v1/composer/suggestions?cwd=%2Fworkspace&kind=file&prefix=%40src%2FApp',
    );
  });
  ...
  it('lists extensions and updates one configured package', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe('/supervisor-api/agents/v1/extensions');
    expect(fetcher.mock.calls[1]?.[0]).toBe('/supervisor-api/agents/v1/extensions/update');
  ...
  it('renames an agent with a validated PATCH request', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      `/supervisor-api/agents/v1/agents/${agentId}`,
      expect.objectContaining({
        method: 'PATCH',
  ...
  it('lists and reads agents with encoded query parameters', async () => {
    ...
    expect(fetcher.mock.calls[0]?.[0]).toBe('/supervisor-api/agents/v1/agents?limit=12&cursor=a%2Fb');
    expect(fetcher.mock.calls[1]?.[0]).toBe(`/supervisor-api/agents/v1/agents/${agentId}`);
  });
  ...
  it('sends run controls and deletes definitions through their own resources', async () => {
    ...
    expect(fetcher.mock.calls.map(([url]) => url)).toEqual([
      `/agents/v1/runs/${runId}/steer`,
      `/agents/v1/runs/${runId}/follow-up`,
      `/agents/v1/agents/${agentId}`,
      `/agents/v1/runs/${runId}/events?afterSequence=6&limit=500`,
    ]);
  });
  ...
  it('streams JSON event frames over WebSocket after the requested sequence', async () => {
    ...
    expect(FakeWebSocket.last?.url).toBe(
      `ws://localhost:3000/supervisor-api/agents/v1/agents/${agentId}/stream?afterSequence=7`,
    );
  });
});
