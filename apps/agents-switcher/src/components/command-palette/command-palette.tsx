...
import { BrowserCommandResult } from './browser-command-result';
...
const isTab = (item: SwitcherItem): item is TabSwitcherItem => item.kind === 'tab';
const isCommand = (item: SwitcherItem): item is BrowserCommandSwitcherItem =>
  item.kind === 'browser-command';
...
export function CommandPalette({
    ...
    async (item: SwitcherItem) => {
      ...
      try {
        if (item.kind === 'tab') {
          ...
          const response = await checkedSend({
            type: 'browser-command/execute',
            command: item.command,
          });
          if (response.type !== 'done')
            throw new Error('Chrome returned an unexpected command result.');
          if (!response.closePalette) {
            await refresh();
            return;
          }
        }
  ...
  const tabs = visibleItems.filter(isTab);
  const commands = visibleItems.filter(isCommand);
                  ...
                  >
                    ...
                    <CommandList className="scroll-py-1">
                      ...
                      {!loading && visibleItems.length === 0 ? (
                        <CommandEmpty>No matching tabs or commands.</CommandEmpty>
                      ) : null}
                      ...
                      {!loading && commands.length > 0 ? (
                        <CommandGroup heading="Browser commands">
                          {commands.map((item) => (
