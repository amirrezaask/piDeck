...
const makeClient = (overrides: Partial<PaletteClient> = {}) => {
  ...
  const items = [
    ...snapshot.tabs.map((tab) => makeTabItem(tab, context)),
    ...Effect.runSync(BrowserCommandsProvider.load(context)),
  ];
  const client: PaletteClient = {
    ...
    send: async (request) => {
      ...
      return {
        ok: true,
        type: 'done',
        closePalette: request.type === 'tab/activate' || request.type === 'browser-command/execute',
      };
    },
    ...overrides,
  };
...
describe('CommandPalette', () => {
  it('renders tabs, focuses search, filters, and shows favicon fallback', async () => {
    ...
    const input = await screen.findByRole('combobox', { name: 'Search tabs and commands' });
    await waitFor(() => expect(input).toHaveFocus());
    expect(screen.getByRole('option', { name: /Dispatch/ })).toBeInTheDocument();
  ...
  it('navigates, activates, closes, pins, and mutes from the keyboard', async () => {
    ...
    const input = await screen.findByRole('combobox', { name: 'Search tabs and commands' });
    await waitFor(() =>
      expect(screen.getByRole('option', { name: /Dispatch/ })).toHaveAttribute(
        'aria-selected',
        'true',
      ),
    );
  ...
  it('closes on Escape and displays empty, error, shortcut warning, and theme states', async () => {
    ...
    await screen.findByText('No matching tabs or commands.');
    expect(screen.getByText('Shortcut unassigned')).toBeInTheDocument();
    expect(container.querySelector('.dark')).not.toBeNull();
    screen.getByRole('combobox', { name: 'Search tabs and commands' }).focus();
