...
import type { BrowserCommandSwitcherItem, TabSwitcherItem } from '/src/domain/switcher-item';
import { BrowserCommandsProvider } from '/src/providers/browser-commands-provider';
...
const commands = Effect.runSync(
  BrowserCommandsProvider.load(context),
) as readonly BrowserCommandSwitcherItem[];
...
describe('fuzzy scoring and ranking', () => {
  ...
  it('combines recency, current-window affinity, and pinned weighting without overpowering text', () => {
    ...
    expect(rankItems([plain, pinned], '', now)[0]?.item.id).toBe(pinned.id);
  });
  it('uses recency for empty queries, command aliases, command prefix, stable ordering, and limits', () => {
    ...
    expect(rankItems([stale, recent], '', now)[0]?.item.id).toBe(recent.id);
    expect(rankItems(commands, 'hotkey', now)[0]?.item.id).toBe('command:open-shortcut-settings');
    expect(
      rankItems([...commands, recent], '> new', now).every(
        ({ item }) => item.kind === 'browser-command',
      ),
    ).toBe(true);
    expect(
      rankItems(
    ...
    expect(rankItems(many, '', now)).toHaveLength(50);
  });
