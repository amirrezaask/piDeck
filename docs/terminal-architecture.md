...
The runtime must support 1,000 durable, mostly idle PTYs, busy interactive terminals, multiple viewers per terminal, browser reconnects, and large retained histories. The host admission cap is now 1,000 and the shared-poller tests exercise descriptor fan-in and reuse, but the complete 1,000-process RSS/latency matrix has not been measured. The cap is therefore an assumed safety limit, not evidence that the scale target has passed.
...
| shared `PtyPoller` with generated registrations                             | shard-owned Unix epoll/kqueue readiness                      | Completed reader-thread removal and FD-reuse fencing; move the process-wide poller into fixed owner shards.                  |
| inclusive PTY byte offsets plus deterministic numeric stream/epoch IDs       | byte-offset `StreamPosition`                                 | Implemented for protocol-v4 snapshot, ready, data, input, resize, and resync paths; retain as an invariant.                   |
...
| protocol-v4 Rust codec plus matching TypeScript capable-client codec         | transport-independent binary protocol                        | Snapshot, ready, data, input, resize, and resync are binary; migrate remaining attach/history/liveness control categories.   |
| owner-side snapshot cut and per-connection attach fence                      | shard-owned atomic snapshot cut                              | Ordering is implemented as attach result, `SNAPSHOT`, `READY`, exact post-cut data; preserve it while moving ownership.      |
| protocol-v4 server `READY` plus owner-side attachment phase                  | server `READY` interaction barrier                           | Implemented for input and resize; preserve independent session and connection phases.                                        |
...
| owner-scheduled hot/warm/parked state with Ghostty compression               | explicit shard processing sets                              | Compression, trimming, transitions, and metrics exist; fixed shards and measured wake-latency gates remain.                  |
| shared-poller descriptor parking with one authority owner thread per session | shard poller ownership                                       | Reader tasks are gone; authority owners must still move to fixed shards to bound idle thread/stack cost.                     |
| assumed 1,000-terminal admission cap                                         | measured 1,000-session budget                                | Do not treat admission as validation; record RSS, FD, poller, shutdown, and latency evidence before claiming the target.      |
