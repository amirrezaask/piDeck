...
export type GhosttyWasmAsset = Uint8Array | URL | string

export type GhosttyWasmSource = {
  readonly terminal: GhosttyWasmAsset
  readonly trampoline: GhosttyWasmAsset
}
...
export class GhosttyRuntime {
  ...
  static async load(source: GhosttyWasmSource): Promise<GhosttyRuntime> {
    const terminalBytes = await loadWasmAsset(source.terminal)
    ...
    const result = await WebAssembly.instantiate(copyToArrayBuffer(terminalBytes), imports);
    instance = result.instance;
    const runtime = new GhosttyRuntime(result.instance);
    await runtime.installWritePtyTrampoline(source.trampoline);
    return runtime;
  }

  call(name: string, ...args: Array<number | bigint>): number {
    const fn = this.exports[name];
    // SAFETY: WebAssembly exports are functions with the ABI signature above.
    // oxlint-disable-next-line anti-slop/no-runtime-typeof
    if (typeof fn !== "function") {
      throw new Error(`libghostty-vt export is unavailable: ${name}`);
    }
    // SAFETY: the export was checked above and the pinned ABI defines its args.
  ...
  private async installWritePtyTrampoline(asset: GhosttyWasmAsset): Promise<void> {
    const trampolineBytes = await loadWasmAsset(asset)
    const result = await WebAssembly.instantiate(copyToArrayBuffer(trampolineBytes), {
      env: {
    ...
    // SAFETY: both values come from the pinned WebAssembly instance/table.
    // oxlint-disable-next-line anti-slop/no-runtime-typeof
...
async function loadWasmAsset(asset: GhosttyWasmAsset): Promise<Uint8Array> {
  if (asset instanceof Uint8Array) return asset
  // oxlint-disable-next-line anti-slop/no-runtime-typeof
  if (typeof fetch !== "function") {
    throw new Error("a fetch-capable loader or Uint8Array WASM source is required")
  }
...
export function loadGhosttyRuntime(source: GhosttyWasmSource): Promise<GhosttyRuntime> {
  runtimePromise ??= GhosttyRuntime.load(source).catch((error) => {
    runtimePromise = null;
    throw error;
  });
