...
const DEFAULT_BROWSER_SERVER: StoredServer = {
  id: 'local',
  name: 'Local',
  // SupervisorClient owns the /agents namespace. Keep the same-origin base at
  // the site root or requests become /agents/agents/v1/*.
  address: '/',
  token: '',
};
...
class BrowserServerConnectionManager implements ServerConnectionManager {
  ...
  private read(): StoredServer[] {
    try {
      ...
      return value.filter(isStoredServer).map((server) =>
        // A short-lived unified-client build persisted /agents as the base URL,
        // even though SupervisorClient appends that namespace itself.
        server.address === '/agents' ? { ...server, address: '/' } : server,
      );
    } catch {
  ...
  client(server: ServerDefinition): SupervisorClient {
    return new SupervisorClient({
      ...
      fetcher: async (input, init) => {
        ...
        const url = new URL(requestUrl, server.address);
