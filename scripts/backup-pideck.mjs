...
if (command === 'create') {
  ...
  const manifest = await verifyBackup(required('--backup'));
  console.log(`Backup verified: ${manifest.files.length} files`);
} else if (command === 'restore') {
  const result = await restoreBackup({ backupDirectory: required('--backup'), databasePath: required('--database'), sessionDirectory: required('--sessions') });
